...
\begin{longtable}{p{0.15\textwidth} p{0.75\textwidth}}
  ...
  & \small An Ethereum test-net that was used to deploy the smart contract to. One of the main benefits was that it provides a fast transaction time for quick feedback. It was chosen over Goerli~\cite{noauthor_goerli_nodate} as the faucet gave out 5 times as much Ether.\\
  Alchemy~\cite{noauthor_alchemy_nodate}
  & \small Alchemy provides useful tools for interacting Sepolia, such as an Ether faucet and an RPC URL so I don't have to run my own Ethereum node.\\
  ...
  & \small A browser-based IDE for writing smart contracts that allows for easy deployment.
  \\
  Ganache CLI~\cite{noauthor_trufflesuiteganache_2023}
  & Ganache CLI was used to create a lightweight, local Ethereum test-net. The tool allowed for recreation of the same private keys for the network so that I could easily repeat automated tests. When compared to Geth, from go-ethereum, Ganache CLI was much more beginner friendly. 
  \\\bottomrule\bottomrule
...
The frontend code was developed from Section~\ref{subsubsec:frontend} to produce a GUI to interact with. Table~\ref{tab:tools-frontend} details the list of tools used.
...
\begin{longtable}{p{0.15\textwidth} p{0.75\textwidth}}
  ...
  & \small A scripting language with many helpful frameworks and libraries that make building a frontend fast and simple.
  \\
  ...
  & \small A reactive, component based web-framework that allows me to create reusable components that react to changes in state and can trigger events at different points in a component's lifecycle.
