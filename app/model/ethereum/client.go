...
func StartClient(addr string) (*ethclient.Client, *accounts.Account, error) {
	...
	once.Do(func() {
		...
		util.Logger.Info("Creating keystore")
		acc, err = CreateKeyStore(viper.GetString("eth.keystore.directory"), viper.GetString("eth.keystore.password"))
		if err != nil {
			util.Logger.Panic(err)
		}
		util.Logger.Info("Created keystore")
